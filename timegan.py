...
def timegan (ori_data, parameters):
  ...
  def embedder (X, T):
    ...
    with tf.compat.v1.variable_scope("embedder", reuse = tf.compat.v1.AUTO_REUSE):
      e_cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell([rnn_cell(module_name, hidden_dim) for _ in range(num_layers)])
      e_outputs, e_last_states = tf.compat.v1.nn.dynamic_rnn(e_cells, X, dtype=tf.float32, sequence_length = T)
      H = keras.layers.Dense(hidden_dim, activation=tf.nn.sigmoid)(e_outputs)   
    return H
  ...
  def recovery (H, T):   
    ...
    with tf.compat.v1.variable_scope("recovery", reuse = tf.compat.v1.AUTO_REUSE):       
      ...
      X_tilde = keras.layers.Dense(dim, activation=tf.nn.sigmoid)(r_outputs)  
    return X_tilde
  ...
  def generator (Z, T):  
    ...
    with tf.compat.v1.variable_scope("generator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      E = keras.layers.Dense(hidden_dim, activation=tf.nn.sigmoid)(e_outputs)   
    return E
  ...
  def supervisor (H, T): 
    ...
    with tf.compat.v1.variable_scope("supervisor", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      S = keras.layers.Dense(hidden_dim, activation=tf.nn.sigmoid)(e_outputs)    
    return S
  ...
  def discriminator (H, T):
    ...
    with tf.compat.v1.variable_scope("discriminator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      Y_hat = keras.layers.Dense(1, activation=None)(d_outputs)
    return Y_hat   
